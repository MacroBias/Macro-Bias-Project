...
export async function POST(request: Request) {
  try {
    ...
    const payload: LegalSectionPayload = {
    ...
    return NextResponse.json({
      section: data ? mapLegalSection(data as LegalSectionRow) : payload,
    });
  } catch (error) {
...
export async function PUT(request: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { message: "Missing legal section id." },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      section: data
        ? mapLegalSection(data as LegalSectionRow)
        : { id, ...payload },
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unexpected error.";
